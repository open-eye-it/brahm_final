...
if(isset($_POST['doc_generate'])){
    ...
    $currentYear = date('Y');
    $list = $reportModel->AllHeadMembersPaidNotDeleted($currentYear);
    if($list['status'] == 'success'){
        ...
        if(count($data) > 0){
            ...
            $doc->output("letters-paid-members" . date("Ymd") . ".doc");
        }else{ ?>
            <script>
        ...
        <script>
            window.location.href = "<?php echo $base_url; ?>"+"letter/letter-paid-member.php";
        </script>
    <?php }
}
?>